...
def fetch_top_records(data_type, table_name, selected_state, selected_year, selected_quarter):
    query = f"SELECT * FROM {table_name}"
...
# If the user selects "All States," set selected_state to None
selected_state = st.selectbox("Select State", list(state_options.keys()), format_func=lambda option: state_options[option])
...
# Fetch top 10 records and display
if data_type.lower() == "transaction": 
    df_top_10 = fetch_top_records(data_type, selected_table, selected_state, selected_year, st.session_state.selected_quarter)
else:
    df_top_10 = fetch_top_records(data_type, selected_table, selected_state, selected_year, st.session_state.selected_quarter)
...
st.subheader(f"Interactive Bar Chart for {display_name} - {selected_year} {selected_quarter}")

# Example bar chart using Plotly Express
bar_fig = px.bar(df_top_10, x='Transaction_type', y='Transaction_count', color='Transaction_type', barmode='group', title=f"{display_name} Bar Chart")
