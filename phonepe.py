...
# # Function to fetch top 10 records from MySQL
# def fetch_top_10_transction_records(region_option, table_name, selected_state, selected_year, selected_quarter):
#     # Modify the query to filter by state if applicable
#     if region_option:
#         query=f"SELECT * FROM {region_option} LIMIT 10 "
        
#         if selected_state:
#             query = f"SELECT * FROM {region_option}  WHERE selected_table = '{table_name}' WHERE State = '{selected_state}' LIMIT 10"
            
#             # if selected_year:
#             #     query = f"SELECT * FROM {table_name} WHERE state = '{selected_state}' AND Year = '{selected_year}' LIMIT 10"
                
#             #     if selected_quarter:
#             #         query = f"SELECT * FROM {}"
#     else:
#         query = f"SELECT * FROM {table_name} WHERE Year = '{selected_year}' LIMIT 10"
...
# # Fetch top 10 records and display
# df_top_10 = fetch_top_10_transction_records(selected_table, selected_state, selected_year, selected_quarter)
# st.dataframe(df_top_10)
...
# Function to fetch top 10 records from MySQL
def fetch_top_10_records(table_name, selected_state, selected_year, selected_quarter ):
    # Modify the query to filter by state if applicable
    if selected_state:
        query = f"SELECT * FROM {table_name} WHERE State = '{selected_state}' LIMIT 10"
        ...
        if selected_year:
            ...
            if selected_quarter:
                query = f"SELECT * FROM {table_name} WHERE state = '{selected_state}' AND Year = '{selected_year}' AND Quarter = '{selected_quarter}' "

    else:
        query = f"SELECT * FROM {table_name} WHERE Year = '{selected_year}' AND Quarter = '{selected_quarter}' "
...
# Fetch top 10 records and display
df_top_10 = fetch_top_10_records(selected_table, selected_state, selected_year, selected_quarter)
st.dataframe(df_top_10)
